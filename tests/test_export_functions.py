...
class TestExportFunctions(unittest.TestCase):
    ...
    def test_exportar_a_pdf_success(self) -> None:
        """Probar exportación a PDF exitosa"""
        filepath = str(Path(self.temp_dir) / 'test_export.pdf')
        ...
        self.assertTrue(Path(filepath).exists(), "El archivo PDF debería existir")
        self.assertGreater(Path(filepath).stat().st_size, 0, "El archivo PDF debería tener contenido")

    def test_exportar_a_pdf_empty_dataframe(self) -> None:
        """Probar exportación a PDF con DataFrame vacío"""
        empty_df = pd.DataFrame()
        filepath = str(Path(self.temp_dir) / 'test_empty.pdf')
    ...
    def test_exportar_a_pdf_invalid_path(self) -> None:
    ...
    def test_exportar_a_sql_success(self) -> None:
        """Probar exportación a SQL exitosa"""
        filepath = str(Path(self.temp_dir) / 'test_export.db')
        ...
        self.assertTrue(Path(filepath).exists(), "El archivo de base de datos debería existir")
    ...
    def test_exportar_a_sql_empty_dataframe(self) -> None:
        """Probar exportación a SQL con DataFrame vacío"""
        empty_df = pd.DataFrame()
        filepath = str(Path(self.temp_dir) / 'test_empty.db')
    ...
    def test_exportar_a_sql_invalid_path(self) -> None:
    ...
    def test_exportar_a_imagen_success(self, mock_qapp) -> None:
        ...
        filepath = str(Path(self.temp_dir) / 'test_export.png')
    ...
    @patch('PySide6.QtWidgets.QApplication')
    def test_exportar_a_imagen_save_failure(self, mock_qapp) -> None:
    ...
    def test_exportar_a_imagen_exception(self) -> None:
    ...
    def test_exportacion_con_datos_especiales(self) -> None:
        ...
        pdf_path = str(Path(self.temp_dir) / 'test_special.pdf')
        ...
        sql_path = str(Path(self.temp_dir) / 'test_special.db')
